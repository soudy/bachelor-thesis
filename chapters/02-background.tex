...
Finally, an overview of the quantum computational circuit model is given along with the description of the \acrlong{qft} and Grover's algorithm.
...
CZ = \czgate{}.
\end{equation}
Generally, a controlled-$U$ operator applies $U$ if the control qubit is \ket{1}.
...
A special class of measurements called projective measurements is sometimes used to simplify calculations.
...
Calculating the expectation value experimentally means preparing and measuring the state multiple times.
...
\Cref{fig:bell-state-circuit} demonstrates a simple quantum circuit which creates a maximally entangled state $\ket{\Phi^+} = \left(\ket{00} + \ket{11}\right)/\sqrt{2}$ from \Cref{eqn:bell-state} and measures both qubits.
...
In quantum computing, the data one can copy is much more restricted.
...
To demonstrate the quantum circuit computational model's ability to solve certain problems more efficiently than classical computers, two quantum algorithms which provide a speedup over their classical counterparts are now reviewed: the \acrlong{qft} and Grover's algorithm.
...
\begin{figure}[ht]
    \begin{adjustwidth}{-1cm}{-1cm}
        ...
        \]
    \end{adjustwidth}
    \caption{The general circuit for the \gls{qft} on $2^n$ amplitudes with $n$ qubits.}
...
Using the diffusion operator $H^{\otimes n}(2\op{0^n}{0^n} - I)H^{\otimes n}$, the amplitudes are inverted about their mean (displayed as the dotted orange line):
\begin{figure}[ht]
    \centering
