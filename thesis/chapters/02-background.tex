...
Formally, if we have functions $f(n)$ and $g(n)$ such that $f(n)$ eventually grows slower than $Mg(n)$ for some real $M$ as $n \to \infty$, we say $f(n) = O(g(n))$.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/complexity-classes-hierarchy.pdf}
...
If a qubit is in a computational basic state, that state can be copied by a $\textnormal{CNOT}$ gate~(\Cref{fig:copy-basis-state}).\footnote{More formally, if a quantum state is in a basis state of a known orthogonal basis, the state can be copied.}
\begin{figure}[ht]
    \large
    \[
    \Qcircuit @C=1em @R=1em @!R {
...
\end{figure}
The quantum state can be thought of as having two registers: the target qubits $\ket{q1\ldots q_n}$ and the oracle qubit \ket{y}.
