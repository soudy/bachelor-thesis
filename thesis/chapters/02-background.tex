...
Note that any single-qubit operator $U$ can be decomposed into these gates, for example $U = R_Z(\gamma)R_Y(\beta)R_Z(\alpha)$ where $\alpha, \beta, \gamma \in \mathbb{R}$.
...
Grover's algorithm is asymptotically optimal: \textcite{zalka1999grover} showed that $O(\sqrt{n})$ is the best we can do for unstructured database search.
