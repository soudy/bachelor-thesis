...
In the context of quantum chemistry, where $H$ describes the Hamiltonian of a system, finding the lowest eigenvalue of the Hamiltonian translates to finding the ground state $\ket{\psi_G} = \ket{\lambda_{\textnormal{min}}}$ and ground state energy $E_G = \lambda_{\textnormal{min}}$ of that system.
...
\end{equation} 
The problem of estimating $\expval{H}$ can be done efficiently on a quantum computer, while being a hard problem to solve classically~\cite{ortiz2001quantum, peruzzo2014variational}.
