...
A previously known quantum algorithm called the \gls{qpe} algorithm solves this problem exponentially faster than the best known classical algorithm~\cite{abrams1999quantum}.
However, the resource requirements to run the \gls{qpe} algorithm greatly exceed the resources available in the near term.
\textcite{peruzzo2014variational} proposed the \gls{vqe} as alternative which can run on \gls{nisq} devices.
This section will review its working and potential applications. 
