...
\begin{equation}
U(H_B, \beta) = e^{-i\beta H_B} = \prod_{j=0}^{n-1} e^{-i\beta X^{(j)}}.
\end{equation}
Note that $e^{-i\beta X}$ is just the $R_x$ operator as defined in \Cref{sec:state-evolution}.
...
\end{equation}
This unitary depends on $2p$ parameters. 
...
\end{equation}
A trial state $\ket{\psi(\vec{\gamma}, \vec{\beta})}$ is then obtained by applying the ansatz to \ket{s}:
\begin{align}
