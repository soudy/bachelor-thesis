...
Rx q[3], 5.156352
    \end{minted}
    \caption[cQASM for the $p$-layer \gls{qaoa} on the graph from \Cref{fig:maxcut-4-graph}.]{
        cQASM for the $1$-layer \gls{qaoa} on the graph from \Cref{fig:maxcut-4-graph} with $\gamma = 3.902484$ and $\beta = 5.156352$.
...
\end{equation}
We then prepare the state $\ket{\psi(\vec{\gamma}_\text{opt}, \vec{\beta}_\text{opt})}$ and measure multiple times in the computational basis to extract the solution, which is the bit string with the highest probability.
