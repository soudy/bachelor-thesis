...
\end{figure}
A $ZZ$ interaction $e^{-i\gamma w_{j, k}(I - Z^{(j)}Z^{(k)})/2}$ from the problem unitary $U(H_L, \gamma)$ (\Cref{eqn:problem-unitary}) is implemented as follows:
\begin{figure}[H]
    \[
...
\begin{figure}[ht]
    ...
    \caption[Experimental results of the \gls{qaoa} on the graph from \Cref{fig:maxcut-4-graph}.]{
        Experimental results of the \gls{qaoa} on the graph from \Cref{fig:maxcut-4-graph}.
